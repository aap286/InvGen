...
def money(num):
    for i in range(len(num)):
        if num[i] == 0:
            ...
            # num[i] = "${}".format(round(num[i],0))
            num[i] = "{:,.0f}".format(round(num[i], 0))
    ...
    for i in range(0, 12):
        ...
        if i == 0:
            ...
            if prevDiff == 0:
                ...
                item[23 + i] = round(
                    schpay + schpay * prevDiff * interest / 365 / 100, 2
                )
            # item[23 + i] = round(item[22] + item[22]*interest/12/100,2)
            ...
            item[23 + i] = round(schpay + schpay * prevDiff * interest / 365 / 100, 2)
            # print(prevDiff)
            # item[23+i] = round(item[22+i] + item[22+i]*interest/12/100,2)
